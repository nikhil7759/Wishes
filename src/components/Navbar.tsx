...
const Navbar: React.FC = () => {
  ...
  const navLinks = [
    ...
    { to: 'https://www.instagram.com/wishes.india/', label: 'Instagram', isExternal: true, isIcon: true, iconType: 'instagram' },
    { to: 'https://www.facebook.com/profile.php?id=61592080582387', label: 'Facebook', isExternal: true, isIcon: true, iconType: 'facebook' },
  ];
            ...
            {navLinks.slice(0, 3).map((link) => (
              link.isExternal ? (
                <a key={link.to} href={link.to} target="_blank" rel="noopener noreferrer" className="hover:opacity-70 transition-opacity whitespace-nowrap uppercase">
                  {link.isIcon ? (link.iconType === 'facebook' ? facebookSvg : instagramSvg) : link.label}
                </a>
              ) : (
          ...
          {navLinks.map((link) => (
            link.isExternal ? (
              <a key={link.to} href={link.to} target="_blank" rel="noopener noreferrer" className="hover:text-[#d1b19a] transition-colors py-2 border-b border-white/10 flex items-center gap-3" onClick={() => setIsOpen(false)}>
                {link.isIcon ? (
                  <>
                    {link.iconType === 'facebook' ? facebookSvg : instagramSvg}
